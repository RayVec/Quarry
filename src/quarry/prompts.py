...
SHARED_SYSTEM_PROMPT = (
    ...
    "  ordered.\n"
    "- Answer the user's question directly rather than defaulting to\n"
    "  document narration unless provenance is itself important.\n"
...
def generation_prompt(request: GenerationRequest) -> str:
    ...
    sections: list[str] = [
        ...
        "## Information Facets\n"
        "The query has been decomposed into these aspects. Use them as\n"
        "coverage checks, not as a required reader-facing outline:\n"
    ...
    sections.append(
        ...
        "- Match the answer shape to the user's request. Respond as\n"
        "  guidance, explanation, comparison, prioritization, or summary\n"
        "  according to the query rather than defaulting to a document tour\n"
        "- Use the facets as internal coverage checks. Organize the answer\n"
        "  around the smallest clear set of reader-facing themes or steps\n"
        "  needed to answer fully\n"
        "- Synthesize aggressively: merge overlapping evidence, summarize\n"
        "  categories, and use only a few representative examples unless\n"
        "  the user explicitly asks for an exhaustive inventory\n"
        "- Prefer the strongest supporting passage for a point. Add more\n"
        "  passages only when they contribute a distinct detail, contrast,\n"
        "  or qualification\n"
        "- If evidence is partial, qualified, or conflicting, state that\n"
        "  plainly\n"
        "- Treat the passages as evidence, not answer prose. Avoid\n"
        "  source-led narration and source-like phrasing unless provenance\n"
        "  or exact wording is materially important\n"
        "- Keep the response compact and readable: use short grouped\n"
        "  paragraphs, avoid laundry-list structure unless requested, and\n"
        "  omit filler or repeated summaries"
    )
