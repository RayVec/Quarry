        ...
        {session ? (
          <div className="drawer-stack">
            <section className="drawer-section">
              ...
              <p data-testid="status-query-status">
                Query status: {session.query_status}
              </p>
              <p data-testid="status-query-stage">
                Current stage: {session.query_stage_label}
              </p>
              <p data-testid="status-runtime-mode">
                Runtime: {session.runtime_mode}
              </p>
              <p data-testid="status-runtime-profile">
                Profile: {session.runtime_profile}
              </p>
              <p data-testid="status-generation-provider">
                Generator: {session.generation_provider}
              </p>
              <p data-testid="status-parser-provider">
                Parser: {session.parser_provider}
              </p>
              <p data-testid="status-refinements">
            ...
            {(session.ui_messages?.length ?? 0) ? (
              <section className="drawer-section">
                ...
                {session.ui_messages.map((message) => (
                  <div
                    className={`message-row ${message.level}`}
              ...
              <section className="drawer-section">
                ...
                {session.retrieval_diagnostics.map((diagnostic) => (
                  <div className="diagnostic-row" key={diagnostic.facet}>
                    ...
                    <p>
                      sparse {diagnostic.sparse.result_count} · dense{" "}
                      {diagnostic.dense.result_count} · fused{" "}
                      {diagnostic.fused_count} · reranked{" "}
                      {diagnostic.reranked_count}
                    </p>
                    {diagnostic.degraded_mode ? (
                      <p>degraded fallback in use</p>
                    ) : null}
                  </div>
                ))}
              </section>
